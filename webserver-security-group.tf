...
resource "aws_vpc_security_group_ingress_rule" "allow_tcp_8080_from_ipv6_internet" {
    security_group_id = aws_security_group.webserver.id
    cidr_ipv6 = "::/0"
    from_port = 8080
    ip_protocol = "tcp"
...
resource "aws_vpc_security_group_ingress_rule" "allow_8000_from_ipv6_internet" {
    security_group_id = aws_security_group.webserver.id
    cidr_ipv6 = "::/0"
    from_port = 8000
    ip_protocol = "tcp"
